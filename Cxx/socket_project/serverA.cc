...
int main(){
    std::ifstream input("backendA.txt");
    dict_t dict = std::move(load_db(input));
    input.close(); // close the file

    UDPServer server = UDPServer("127.0.0.1", PORT_SERVER_A).bind();
    std::cout << "The Server A is up and running using UDP on port " << server.getPort() << "." << std::endl;
    ...
    while(1) {
        ...
        Query *query = (Query *) p;
        std::cout << "The Server A received input <" << query->m_input
        ...
        switch(query->m_type){
            case search: {
                ...
                std::cout << "The Server A has found < " <<  match_cnt
            ...
            case prefix: {
                ...
                std::cout << "The Server A has found < " << match_cnt << " > matches" << std::endl;
                break;
            }
            case suffix: {
...
//        std::clog <<"Server-A: " << result << std::endl;
